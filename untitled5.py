...
#st.sidebar.title('Simulated microgrid structure')
...
with col4:
         ...
         st.image("https://raw.githubusercontent.com/TeckVo/GUI-design/main/Figure_set/Picture7.png")
with col5:
         st.subheader("Optimization framework")
         st.image("https://raw.githubusercontent.com/TeckVo/GUI-design/main/Figure_set/Picture6.png")
...
line_chart_3 = alt.Chart(df_2).mark_line().encode(
         ...
         color=alt.Color('Output:N', legend=alt.Legend(orient='bottom'))).properties(title='Output power from solar panel system', width=220, height=200)
...
line_chart_4 = alt.Chart(df_4).mark_line().encode(
         ...
         color=alt.Color('Solar irradiance:N', legend=alt.Legend(orient='bottom'))).properties(title='Solar irradiance data during one year', width=220, height=200)
